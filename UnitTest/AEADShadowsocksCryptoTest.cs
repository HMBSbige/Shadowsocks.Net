...
public class AEADShadowsocksCryptoTest
{
	...
	private static void IsSymmetricalTcp(string method, string password, int size)
	{
		...
		encryptor.EncryptTCP(buffer, output[..(encryptor.SaltLength + 1)], out int p, out int o);
		Assert.AreEqual(0, p);
		Assert.AreEqual(encryptor.SaltLength, o);

		encryptor.EncryptTCP(buffer, output[o..], out int p0, out int o0);
		Assert.AreNotEqual(0, p0);
		Assert.AreNotEqual(0, o0);

		ReadOnlySequence<byte> encBuffer = new(output[..(o0 + o)].ToArray());
